...
export default function DriverCareForm() {
  ...
  const postQuery = async () => {
    ...
    if (data["ResultCode"] == "1000") {
      ...
      const useCouponRequest = {
        acctid: parseInt(OrderData.AcctId),
  ...
  return (
    <LayoutMobile title={Service + "需求單"} bgColor={bgColor}>
      ...
      <div className={"w-full pt-6 px-4 text-3xl text-gray-700"}>
        {Service + "需求單"}
      </div>
      <div className={"w-full py-2 px-4 " + fontColor}>
        為確保服務順利使用，請填寫真實資料
      </div>
      {acctid != 0 && couponOptions.length > 0 && (
        <div>
          <div className="w-full px-4 py-2">
            <span className={fontColor}>優惠券</span>
          </div>
      ...
      <div className={"w-full " + bgColor}>
        <div className="w-full p-4">
          <span className={fontColor}>預約人資料</span>
      ...
      <div className={"w-full " + bgColor}>
        <div className="w-full p-4">
          <span className={fontColor}>乘車服務資料</span>
        </div>
        <div className="w-full p-4 bg-white">
          <DateTimeLocalPicker
            dateTitle="司機報到日期"
      ...
      <div className={"w-full " + bgColor}>
        <div className="w-full p-4 flex justify-between ">
          <span className={fontColor + " w-full"}>乘車人資料</span>
          <CheckBox
            label="同預約人"
            name="asOrder"
            checked={asOrder}
